...
template <typename T>
struct FocalLossGradFunctor<CPUDevice, T> {
...
class FocalLossGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    FocalLossGradFunctor<Device, T> functor;
...
#if GOOGLE_CUDA
template <typename T>
class FocalLossGradOp<GPUDevice, T> : public OpKernel {
 public:
  explicit FocalLossGradOp(OpKernelConstruction* context) : OpKernel(context) { }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    FocalLossGradFunctorGPU<T>()(context->eigen_device<GPUDevice>(), logits_in.matrix<T>(), labels_in.vec<T>(), softmax_tensor.matrix<T>(), alpha_vec, gamma_in.scalar<T>(), loss_in.vec<T>(), grad_out->matrix<T>());
...
#define REGISTER_CPU(T)                                          \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("FocalLossGrad").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      FocalLossGradOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(int32);
...
#define REGISTER_GPU(T)                                          \
  ...
  extern template struct FocalLossGradFunctorGPU<float>;                \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("FocalLossGrad").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      FocalLossGradOp<GPUDevice, T>);
REGISTER_GPU(float);
#endif  // GOOGLE_CUDA
